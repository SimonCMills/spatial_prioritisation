...
# extract point identifier (for grouping by point) from data and generate 
# point level communities
pred_data <- fit$data[fit$data$rep_index1 != -99,]
point_key <- pred_data %>% 
    mutate(cl = gsub("^.*__cluster_", "", sp_cl)) %>%
    select(cl, time) %>%
    unique %>%
    group_by(cl) %>%
    mutate(point_id = paste0(cl, "__", 1:n()))
pred_data <- left_join(pred_data, point_key)

preds <- fitted_flocker(fit, type = "occupancy", 
                        new_data = pred_data, 
                        summarise = FALSE)

preds_full <- cbind(pred_data, preds)
saveRDS(preds_full, "outputs/preds_full_30-01-2023.rds")

# data
# Trait data
HWI_dat <- read.csv("data/Dataset_HWI.csv") %>% 
  mutate(Species.name = str_replace(Species.name, " ", "_"), IUCN.name = str_replace(IUCN.name, " ", "_"))

Elton_dat <- read.delim("data/BirdFuncDat.txt") %>% 
  mutate(Scientific = str_replace(Scientific, " ", "_"))# %>% 
  # select(Scientific, Nocturnal)

names(preds_full)
#### Bind all traits ####
Trait_occ <- left_join(preds_full, HWI_dat, by = c("phylo" = "Tree.name")) %>%
    left_join(., Elton_dat, by = c("phylo" = "Scientific"))
...
Trait_db <- Trait_occ %>% ungroup() %>%
  ...
  # mutate(species = ifelse(species %in% c("Grallaria_saturata__JBS", "Grallaria_saturata__SCM"), 
  #                         "Grallaria_saturata", species)) %>% 
  ## Select only species that are in the filter dataset
  # filter(species %in% rownames(Bird_comm_filter)) %>% #? check with Oscar
  ## Select traits
  group_by(species,
           ## Diet
